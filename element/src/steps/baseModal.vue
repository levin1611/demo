      ...
      <transition name="fade">
        <v-step v-if="tour.currentStep === index" v-for="(step, index) of tour.steps" :key="index" :step="step" :previous-step="tour.previousStep" :next-step="tour.nextStep" :stop="tour.stop" :is-first="tour.isFirst" :is-last="tour.isLast" :labels="tour.labels" :highlight="tour.highlight" :style="stepStyle">
          ...
          <div slot="content" style="text-align:left;font-size: 14px;color:rgba(0,0,0,0.8);line-height: 16px;margin: 12px 0">
            <div v-if="tour.steps[tour.currentStep].content" v-html="tour.steps[tour.currentStep].content"></div>
          </div>
          <div slot="actions">
            <div class="_f _r _j_b _a_c" style="margin-top: 20px">
              <div style="font-size: 14px">{{tour.currentStep+1}}/{{tour.steps.length}}</div>
              <div>
                <!-- <Button v-if="!tour.isLast" @click="tour.stop" class="v-step__button">{{$t('guide.skip')}}
                </Button> -->
                <Button v-if="!tour.isFirst" @click.stop="tour.previousStep">{{$t('guide.lastStep')}}</Button>
                <Button v-if="!tour.isLast" @click.stop="tour.nextStep" type="primary"  >{{$t('guide.nextStep')}}</Button>
                <Button v-if="tour.isLast" type="primary"  @click="tour.stop">
                  {{$t('guide.start')}}
                </Button>
              </div>
            </div>
...
<script type="text/javascript">
  ...
  // | 'left-end'
    export default {
        ...
        computed: {
            stepStyleHeader() {
                const o = { lineHeight: '24px', fontSize: '18px' };
                return o;
            },
            stepStyle() {
                const o = { maxWidth: '660px', padding: '20px 30px', borderRadius: '8px' };
