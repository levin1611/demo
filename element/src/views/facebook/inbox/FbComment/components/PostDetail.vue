<template>
    <div class="post-detail" :style="`height:${bodyHeight}px`">
        <div class="post-detail-header">
            ...
            <div class="post-detail-header-status">
                <crmArchiveStatus @showDetail="$emit('changeShowDetail', true)" :status="commentAndPost.clue_status"></crmArchiveStatus>
            </div>
...
<script>
    ...
    export default {
        ...
        components: { testModal, SelfUpload, crmArchiveStatus },
        data() {
            return {
                attachments: [],
                subattachments: [],
        ...
        computed: {
            ...
            content_height() {
                return this.bodyHeight - 60 - 100 - 40;
            },
            commentList() {
        ...
        methods: {
            ...
            changeInputMsg(msg) {
                ...
                const dom = document.getElementById('fbCommentBox');
                    ...
                    if (this.inputMessage.replace(/(^\s*)|(\s*$)/g, '') !== '') { // 发送内容不为空和空格
                        ...
                        util.ajaxInternational({
                            ...
                            data: {
                                ...
                                recipientId: this.commentAndPost.commentator_id,
                                recipientName: this.commentAndPost.commentator_name,
                                recipientAvatar: this.commentAndPost.commentator_avatar,
                                id: this.post.id,
                                accessToken: this.pageAccessToken,
        ...
        watch: {
            ...
            commentVerbalInfo(val) {
                // console.log(this.commentVerbalInfo);
                if (val.length) {
                    this.changeInputMsg(val[val.length - 1]);
                }
            },
            commentSecondVerbalInfo(val) {
                // console.log(this.commentSecondVerbalInfo);
                if (val.length) {
                    this.changeCommentInputMsg(val[val.length - 1]);
                }
            }
        }
    };
