<template>
    <div class="split-mail-table" v-loading="handleLoading">
        ...
        <div class="split-search-header">
            ...
            <ul v-show="!selectMailList.length && currentPersonId === userId"
        ...
        <MailArchive :visible.sync="visibleArchive"
            ...
            @archiveFinished="handleArchiveFinished">
        </MailArchive>
        <!-- 是否全部标记提示 -->
        <Modal
            top="40vh"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            /** 跳转到邮件详情页 */
            goMailDetail(item, e) {
                if (e && e.target.className.indexOf('mail_list_state_task') >= 0) return;
                if (this.selectedMailId === item.id) return;
            ...
            handle_search_data(mailList, setData) {
                mailList = mailList.map(item => {
                    ...
                    const mailInfo = {
                        ...
                        setTaskFlag: item.setTaskFlag || 0
                    };
            ...
            handle_table_data(mailList, setData) {
                for (let i = 0; i < mailList.length; i++) {
                    const data = {
                        ...
                        setTaskFlag: mailList[i].setTaskFlag || 0
                    };
                    setData.push(data);
                }
                    ...
                    if (ids.length) {
                        util.ajaxMailJson({
                            ...
                            if (response.data.code === 1) {
                                ...
                                this.handleUpdateTableData({ key: 'fav', val: `${flag}`, ids });
                                this.$store.dispatch('getUnreadMail');
            ...
            readAll() {
                util.ajaxMailJson({
                    ...
                    if (response.data.code === 1) {
                        this.$store.dispatch('getUnreadMail');
                        const ids = this.getSupportOperateIds([2, 3, 5], this.mailTableData).ids;
                        this.handleUpdateTableData({ key: 'fav', val: '1', ids });
            ...
            changeFinishedAll() {
                ...
                util.ajaxMailJson({
                    ...
                    if (response.data.code === 1) {
                        this.$store.dispatch('getUnreadMail');
                        const ids = this.getSupportOperateIds([2, 3, 5], this.mailTableData).ids;
                        this.handleUpdateTableData({ key: 'mailToDoFlag', val: 2, ids });
                ...
                if (ids.length) {
                    util.ajaxJson({
                        ...
                        if (res.data.code === 1) {
                            ...
                            this.handleUpdateTableData({ key: 'mailToDoFlag', val: status, ids });
                            this.$store.dispatch('getUnreadMail');
                            switch (status) {
                                case 1:
            ...
            flag_star(isFlag, arr = this.selectMailList) {
                ...
                if (ids.length) {
                    util.ajaxMailJson({
                        ...
                        this.handleUpdateTableData({ key: 'mailStarFlag', val: `${isFlag}`, ids });
                    }).catch(() => {
                        this.$Message.error(this.$t('mail.flag_star_error'));
                    });
                } else {
            ...
            saveTag() {
                ...
                util.ajaxMailJson({
                    ...
                    if (res.data.code === 1) {
                        ...
                        if (this.selectMailList.length > 0) {
                            this.signLabel(res.data.data.id);
                        }
                    } else {
                ...
                if (ids.length) {
                    util.ajaxMailJson({
                        ...
                        if (res.data.code === 1) {
                            ...
                            this.handleUpdateTableData({ key: 'mailLabel', val: `${tagId}-1`, ids });
            ...
            mailDelLabel(mailId, tagId) {
                util.ajaxMailJson({
                    ...
                    if (res.data.code === 1) {
                        this.handleUpdateTableData({ key: 'mailLabel', val: `${tagId}-0`, ids: [mailId] });
                    } else {
                ...
                if (this.selectMailList.length === 0) {
                    ...
                    util.ajaxMailJson({
                        ...
                        if (response.data.code === 1) {
                            ...
                            this.handleUpdateTableData({ key: 'moveMailBoxType', ids: this.selectMailIds });
                        } else {
                ...
                if (ids.length) {
                    util.ajaxMailJson({
                        ...
                        if (res.data.code === 1) {
                            ...
                            this.handleUpdateTableData({ key: 'moveMailBoxType', ids: ids });
                            this.$store.dispatch('getUnreadMail');
                        } else {
                            this.$Message.error(this.$t('mail.deleteErrorTip'));
                        }
            ...
            handleArchiveFinished() {
                this.handleUpdateTableData({ key: 'archiveFlag', val: 1, ids: this.selectMailIds });
            },
            /* 部门树结构的渲染函数 */
                ...
                if (ids.length) {
                    ...
                    util.ajaxMailJson({
                        ...
                        if (res.data.code === 1) {
                            ...
                            this.handleUpdateTableData({ key: 'mailForwardedInternallyFlag', val: 1, ids });
                        } else {
            ...
            updateStatus(clueId, status) {
                ...
                this.showClue({ clueId, type: 1, currentMailInfo: { mailAddress: this.mailAddress } });
            ...
            async showClue({ clueId, rightUserIds, createUserIds, type, currentMailInfo }) {
                if (this.currentMailInfo.mailAddress === currentMailInfo.mailAddress) return;
                this.currentMailInfo = Object.assign({ noHaveManageAuthority: true }, currentMailInfo);
                this.mailAddress = '';
                if (type === 0) {
            ...
            handleCancel() {
                ...
                tableApi.mailCancel({
                    ...
                    if (res.code === 1) {
                        ...
                        this.handleUpdateTableData({ key: 'successFlag', val: '4', ids: this.selectMailIds });
                    } else {
            ...
            refreshDataList(id) {
                mailCommon.getMailTask({
                    ...
                    if (response.code === '1' && response.data.length) {
                        this.handleUpdateTableData({ key: 'setTaskFlag', val: 1, ids: [id] });
                    } else {
                        this.handleUpdateTableData({ key: 'setTaskFlag', val: 0, ids: [id] });
                    }
                });
            },
            // littleDrawer点击事件传递
            ...
            handleKeyDown(e) {
                if (this.selectedMailIndex === -1) return;
                if ([38, 40].includes(e.keyCode)) {
                    ...
                    const mailListContainer = this.$refs.mailListContainer;
                    const activeMailDOM = mailListContainer.querySelector('.selected-mail-item');
                    // 38 键盘上
                    if (e.keyCode === 38) {
                        if (this.selectedMailIndex > 0) {
                            this.debounceTimer = setTimeout(() => {
                                const item = this.mailTableData[this.selectedMailIndex - 1];
                                this.goMailDetail(item);
                            }, 150);
                            // 处理左侧列表滚动条
                            if (activeMailDOM.offsetTop < mailListContainer.scrollTop + activeMailDOM.scrollHeight) {
                                mailListContainer.scrollTop = activeMailDOM.offsetTop - activeMailDOM.scrollHeight;
                            } else {
                                if (!Utils.isInViewPort(activeMailDOM, mailListContainer)) {
                                    if (activeMailDOM.offsetTop > mailListContainer.scrollTop + mailListContainer.offsetHeight) {
                                        mailListContainer.scrollTop = (activeMailDOM.offsetTop + activeMailDOM.scrollHeight) - mailListContainer.offsetHeight + 6;
                                    } else {
                                        mailListContainer.scrollTop = activeMailDOM.offsetTop - activeMailDOM.scrollHeight;
                                    }
                                }
                        ...
                        if (this.selectedMailIndex < (this.mailTableData.length - 1)) {
                            this.debounceTimer = setTimeout(() => {
                                const item = this.mailTableData[this.selectedMailIndex + 1];
                                this.goMailDetail(item);
                            }, 150);
                            if ((mailListContainer.scrollTop + mailListContainer.offsetHeight) - activeMailDOM.offsetTop < activeMailDOM.scrollHeight) {
                                mailListContainer.scrollTop = (activeMailDOM.offsetTop + activeMailDOM.scrollHeight) - mailListContainer.offsetHeight + 6;
                            } else {
                                if (!Utils.isInViewPort(activeMailDOM, mailListContainer)) {
                                    if (activeMailDOM.offsetTop > mailListContainer.scrollTop + mailListContainer.offsetHeight) {
            ...
            // 无感刷新相关 邮件标签单独处理
            handleUpdateTableData({ key, val, ids }) {
                if (((this.currentBox === 'unreadBox' && key === 'fav' && val === '1') ||
                    (this.currentBox === 'star_box' && key === 'mailStarFlag' && val === '0') ||
                    (this.currentBox === 'untreated' && key === 'mailToDoFlag' && val === 2) ||
                    key === 'moveMailBoxType') &&
                    ids.length === this.mailTableData.length) {
                    setTimeout(() => {
                    ...
                    if ((this.currentBox === 'unreadBox' && key === 'fav' && val === '1') ||
                        ...
                        if (this.currentBox !== 'search') {
                            ids.forEach(id => {
                                const tableIndex = this.mailTableData.findIndex(list => list.id === id);
                                if (tableIndex !== -1) {
                                    this.mailTableData.splice(tableIndex, 1);
                                }
                            });
                        }
                        setTimeout(() => {
                        ...
                        // 邮件标签单独处理 包括标签的增加删除
                        if (key === 'mailLabel') {
                            const labelId = val.split('-')[0];
                            const handleType = val.split('-')[1];
                            const tagList = this.tagList.filter(item => `${item.id}` === labelId);
                            if (this.currentBox === `tag_${labelId}` && handleType === '1') {
                                this.emptySelection();
                            } else {
                                ids.forEach(id => {
                                    const tableIndex = this.mailTableData.findIndex(list => list.id === id);
                                    if (tableIndex !== -1) {
                                        if (handleType === '1') {
                                            if (!this.mailTableData[tableIndex].labelId.includes(labelId)) {
                                                this.mailTableData[tableIndex].labelId.push(`${tagList[0].id}`);
                                                this.mailTableData[tableIndex].labelColor.push(tagList[0].labelColor);
                                                this.mailTableData[tableIndex].labelName.push(tagList[0].labelName);
                                            }
                                        } else {
                                            if (this.currentBox === `tag_${labelId}`) {
                                                this.mailTableData.splice(tableIndex, 1);
                                                setTimeout(() => {
                                                    this.updateTableData();
                                                }, 800);
                                            } else {
                                                const tagIndex = this.mailTableData[tableIndex].labelId.findIndex(item => item === labelId);
                                                this.mailTableData[tableIndex].labelId.splice(tagIndex, 1);
                                                this.mailTableData[tableIndex].labelColor.splice(tagIndex, 1);
                                                this.mailTableData[tableIndex].labelName.splice(tagIndex, 1);
                                            }
                                        }
                                    }
                                });
                                if (this.selectedMailId && ids.includes(this.selectedMailId)) {
                                    this.$refs.mailDetail.getTagsById();
                                }
                            }
                        } else if (key === 'writeMail') { // 写信单独处理
                            if (this.currentBox === 'draftBox') {
                                if (val === 'sendTimedEmail') {
                                    this.updateTableData();
                                } else {
                                    ids.forEach(id => {
                                        const tableIndex = this.mailTableData.findIndex(list => list.id === id);
                            ...
                            } else if (['groupBox', 'sentBox'].includes(this.currentBox) && !this.groupBoxId) {
                                setTimeout(() => {
                                    this.updateTableData();
                                }, 800);
                                if (this.selectedMailId && ids.includes(this.selectedMailId)) {
                                    this.$refs.mailDetail.mailDetailinit();
                                }
                            } else {
                                if (['replyCurrent', 'replyAll', 'replyWithAttachment'].includes(val)) {
                                    ids.forEach(id => {
                                        const tableIndex = this.mailTableData.findIndex(list => list.id === id);
                                        if (tableIndex !== -1) {
                                            this.$set(this.mailTableData[tableIndex], 'mailAnsweredFlag', 1);
                                            if (!this.groupBoxId || this.mailTableData[tableIndex].mailToDoFlag === 1) {
                                                this.$set(this.mailTableData[tableIndex], 'mailToDoFlag', 2);
                                            }
                                        }
                                    });
                                } else if (val === 'forward') {
                                    ids.forEach(id => {
                                        const tableIndex = this.mailTableData.findIndex(list => list.id === id);
                                        if (tableIndex !== -1) {
                                            this.$set(this.mailTableData[tableIndex], 'forwardFlag', 1);
                                        }
                                    });
                                }
                                if (this.selectedMailId && ids.includes(this.selectedMailId)) {
                            ...
                            ids.forEach(id => {
                                const tableIndex = this.mailTableData.findIndex(list => list.id === id);
                                if (tableIndex !== -1) {
                                    this.$set(this.mailTableData[tableIndex], key, val);
                                }
                            });
                            if (this.selectedMailId && ids.includes(this.selectedMailId)) {
                                if (key !== 'fav') {
                                ...
                                if (key === 'archiveFlag') {
                                    this.$refs.mailDetail.getMailArchiveData();
                                }
                            }
                        }
                    }
            ...
            updateTableData() {
                if (this.currentBox === 'search') {
                                    ...
                                    if (res.data.data.length) {
                                        ...
                                        res.data.data.forEach((item) => {
                                            ...
                                            const mailInfo = {
                                                ...
                                                date: this.dateFormat('yyyy-MM-dd hh:mm', item.mailSendDate),
                                                wholeDate: this.dateFormat('yyyy-MM-dd hh:mm:ss', item.mailSendDate),
                                                fromOrToFlag: item.fromOrToFlag, // 收信还是发信  1-发信
                                                fav: item.mailReadFlag,
                                                originalMailBoxTypeId: item.originalMailBoxTypeId, // 邮件原始来源邮箱
                                        ...
                                        this.mailTableData = setData;
                                        this.getCurrentPageEmails(this.mailTableData);
                                        this.getCompanyStatusByEmails();
                                    }
                                }
                            });
                    ...
                    util.ajaxMailJson({
                        ...
                        if (response.data.code === 1 && response.data.data.content && response.data.data.content.length) {
                            if (!response.data.data.content.length && this.currentPage > 1) {
                                this.$store.commit('setCurrentPage', this.currentPage - 1);
                                this.updateTableData();
                            }
                                ...
                                const data = {
                                    ...
                                    theme:
                                        item.mailSubject === ''
                                            ...
                                            : item.mailSubject,
                                    sender: item.mailFrom,
                                    recipient: item.mailTo && item.mailTo[0] === '[' && item.mailTo[item.mailTo.length - 1] === ']' ? JSON.parse(item.mailTo) : (item.mailTo ? item.mailTo.split(',') : ''),
                                    // date: item.mailSendDate ? this.dateFormat('yyyy-MM-dd', item.mailSendDate) : '',
                                    wholeDate: item.mailSendDate ? this.dateFormat('yyyy-MM-dd hh:mm:ss', item.mailSendDate) : '',
                                    fromOrToFlag: item.fromOrToFlag, // 收信还是发信  1-发信
                                    fav: item.mailReadFlag, // 0-未读   1-已读
                                    originalMailBoxTypeId: item.originalMailBoxTypeId, // 邮件原始来源邮箱
                                    mailCrmFlag: item.mailCrmFlag, // 发件人是否在crm系统中存在
                                    clueFlag: `${item.mailMyclueFlag}`, // 发件人是否在线索中存在
                                    recieverCrmFlag: item.recieverCrmFlag, // 收件人是否在crm系统中存在
                                    ...
                                    mailGroupSendingSingleShowFlag:
                                        item.mailGroupSendingSingleShowFlag, // 群发单显
                                    mailTraceFlag: item.mailTraceFlag, // 追踪
                                    recentOpenIp: item.recentOpenIp, // 追踪气泡中最近打开位置IP
                                    recentOpenPosition: item.recentOpenPosition, // 追踪气泡中最近打开位置
                                    recentOpenTime: item.recentOpenTime
                                    ...
                                    mailStarFlag: item.mailStarFlag, // 是否有星标
                                    mailTimeFlag: item.mailTimeFlag, // 是否有定时邮件
                                    mailBoxTypeId: item.mailBoxTypeId, // 邮件属于哪个箱
                                    mailDraftFlag: item.mailDraftFlag, // 邮件是不是草稿
                                    createUserId: item.createUserId,
                                    labelColor: item.labelColor
                                        ? item.labelColor.split(',')
                                        : [],
                                    labelId: item.labelId ? item.labelId.split(',') : [],
                                    labelName: item.labelName
                                        ? item.labelName.split(',')
                                        : [],
                                    mailAnsweredFlag: item.mailAnsweredFlag, // 邮件是否被回复
                                    mailSendErrorMsg: item.mailSendErrorMsg, // 邮件发送失败原因
                                    replyTo: item.replyTo && item.replyTo[0] === '[' && item.replyTo[item.replyTo.length - 1] === ']' ? JSON.parse(item.replyTo)[0] : (item.replyTo ? item.replyTo.split(',')[0] : ''), // 指定回复邮箱
                                    mailResumes: item.mailResumes, // 邮件正文内容
                                    mailToDoFlag: item.mailToDoFlag, // 邮件的处理状态 0--未设置  1--待处理  2--已完成
                                    companyName: this.mailTableData[index] ? this.mailTableData[index].companyName : '', // 邮箱对应的公司名称
                                    contactsName: this.mailTableData[index] ? this.mailTableData[index].contactsName : '', // 邮箱对应的客户名称
                                    shortName: this.mailTableData[index] ? this.mailTableData[index].shortName : '', // 公司简称
                                    forwardFlag: item.forwardFlag, // 是否已转发
                                    archiveFlag: item.archiveFlag, // 是否归档
                                    oppositeAnsweredFlag: item.oppositeAnsweredFlag, // 对方已回复
                                    mailInterForwardingFlag: item.mailInterForwardingFlag, // 邮件是否来自于内部转发 0---不是 1---是
