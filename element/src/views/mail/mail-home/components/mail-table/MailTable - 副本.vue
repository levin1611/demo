<template>
    <div class="mail-table" v-loading="handleLoading">
        <!-- 表格上方操作栏 -->
        <div class="header clearfloat">
            ...
            <ul v-show="currentPersonId === userId && !selectMailList.length && currentBox!=='groupBox' && !groupBoxId" class="operates clearfloat">
                <!-- 标记 -->
            ...
            <ul v-show="selectMailList.length" class="operates clearfloat">
                <!-- 标记 -->
                <li v-if="!signFlag && !['dustbin', 'trash', 'draftBox'].includes(currentBox)">
                    <Dropdown trigger="click" @command="changeReadState">
                        <Tooltip placement="top" transfer :content="$t('mail.mark')">
            ...
            <div class="view-box">
                <Tooltip :content="$t('crm.Table.viewList')" effect="dark" placement="top">
                    <span class="change-customer-view change-customer-view-left piwik_view_list change-customer-view-active" @click="handleReadModeSet(1)">
                        <i class="custom custom-my-customer-view-list"></i>
                    </span>
                </Tooltip>
                <Tooltip :content="$t('crm.Table.viewCard')" effect="dark" placement="top">
                    <span class="change-customer-view change-customer-view-right piwik_view_card" @click="handleReadModeSet(2)">
            ...
            <FilterTags
                ...
                style="top: 0px; left: 8px; height: 34px"
                :order_column="sort"
                :order_type="sortType"
            ...
            <Table
                ...
                <TableColumn
                    ...
                    <template slot-scope="{ row }">
                        <div class="period" v-if="(item.key === 'checkbox' ||  (currentBox==='groupBox' &&  item.key === 'subject')) && !groupBoxId">
                            <span>{{ row.title }}</span>
                            <span>
                                <Icon
                        ...
                        <template v-if="item.key === 'expandColumn' && row.data.length && row._expanded">
                            <TableTemplate
                                ...
                                @groupBoxHandle="groupBoxHandle">
                            </TableTemplate>
                        </template>
                    </template>
                </TableColumn>
                <TableColumn
                    v-if="currentBox !== 'groupBox' || groupBoxId"
                    fixed="right"
                    width="25"
                    align="center"
        ...
        <MailDetail
            ...
            :id="id_previewMail"
            type="mailTable"
            originalType="mailTable"
            :modalWidth="'80%'"></MailDetail>
    </div>
</template>
...
<script>
    ...
    import { getIsEnable, enforceRules, GET_MAILBOX_TYPE } from '@/api/mail.js';
    ...
    export default {
        ...
        computed: {
            ...
            tableHeight() {
                if (this.currentBox === 'untreated') {
                    return this.window_height - 200;
                } else {
                    return this.window_height - 160;
                }
            },
            no_data_text() {
        ...
        data() {
            return {
                ...
                expandColumns: [
                    ...
                    {
                        title: this.groupBoxId ? this.$t('mail.mailTable.title7') : this.$t('mail.time'),
                        label: this.groupBoxId ? this.$t('mail.mailTable.title7') : this.$t('mail.time'),
                        key: 'date',
                        width: 120,
                        minWidth: 120,
                        sortable: 'custom'
                ...
                mailStatusFilterOptions: [
                    {
                        ...
                        filterOptions: [
                            {
                                title: this.$t('mail.unread'),
                                value: 100
                            },
                            {
                                title: this.$t('mail.start'),
                                value: 101
                            },
                            {
                                title: this.$t('mail.untreated'),
                                value: 102
                            },
                            {
                                title: this.$t('mail.hasAttachment'),
                                value: 103
                            },
                            {
                                title: '未回复',
                            ...
                            // {
                            //     title: this.$t('mail.alreadForward'),
                            //     value: 104
                            // },
                            // {
                            //     title: this.$t('mail.forwarding'),
                            //     value: 105
                            // },
                            // {
                            //     title: this.$t('mail.forwardedInternally'),
                            //     value: 106
                            // },
                            // {
                            //     title: this.$t('mail.alreadarchiveToCRM'),
                            //     value: 107
                            // }
                        ]
                    },
                ...
                lang_mail: {
                    ...
                    sendCount: this.$t('mail.mailTable.title1'),
                    sendedCount: this.$t('mail.mailTable.title2'),
                    sendedRate: this.$t('mail.mailTable.title3'),
                    openCount: this.$t('mail.mailTable.title4'),
                    openRate: this.$t('mail.mailTable.title5'),
                    createTime: this.$t('mail.mailTable.title6')
                },
                /* 多语言 */
                lang_contact: {
                ...
                BulkMailTableColumns: [
                    ...
                    {
                        title: this.$t('mail.mailTable.title1'),
                        label: this.$t('mail.mailTable.title1'),
                        key: 'sendCount',
                        width: 120,
                        minWidth: 120
                    },
                    {
                        title: this.$t('mail.mailTable.title2'),
                        label: this.$t('mail.mailTable.title2'),
                        key: 'sendedCount',
                        width: 120,
                        minWidth: 120
                    },
                    {
                        title: this.$t('mail.mailTable.title3'),
                        label: this.$t('mail.mailTable.title3'),
                        key: 'sendedRate',
                        width: 120,
                        minWidth: 120
                    },
                    {
                        title: this.$t('mail.mailTable.title4'),
                        label: this.$t('mail.mailTable.title4'),
                        key: 'openCount',
                        width: 120,
                        minWidth: 120
                    },
                    {
                        title: this.$t('mail.mailTable.title5'),
                        label: this.$t('mail.mailTable.title5'),
                        key: 'openRate',
                        width: 120,
                        minWidth: 120
                    },
                    {
                        title: this.$t('mail.mailTable.title6'),
                        label: this.$t('mail.mailTable.title6'),
                        key: 'createTime',
                        width: 180,
                        minWidth: 120
                    },
                    {
                        title: this.$t('formSet.operate'),
                        label: this.$t('formSet.operate'),
                        key: 'operate',
                        width: 120,
                        minWidth: 120,
                        resizable: false
        ...
        methods: {
            ...
            handleExpandChange(e) {
                ...
                if (e._expanded) {
                    this.$nextTick(() => {
                        ...
                        const tempArr = this.table_list.find((item) => item.desc === e.desc).data;
            ...
            init(retainSelectedMails) {
                ...
                if (this.currentBox && this.currentBox !== 'search') {
                    if (this.currentBox !== 'groupBox' || this.groupBoxId) {
                        ...
                        this.BulkMailTableColumns = this.BulkMailTableColumns.map((col, index) => {
                            col.show_filter_content = false;
                            col.filtering = false;
                            if (col.key && !expandTemp.includes(col.key)) {
                                col.renderHeader = this.renderBulkHeader_filter(index);
                            }
                            return col;
                        });
                    }
            ...
            getFieldOrderList() {
                 ...
                 * @date 2020/8/25
                 */
                if (!['search'].includes(this.currentBox)) {
                    util.ajaxJson({
                        ...
                        if (response.data.code === '1') {
                            if (response.data.data) {
                                ...
                                // if (this.currentBox !== 'groupBox') {

                                // }else{
                                //     this.parentColumns = this.BulkMailTableColumns;
                                //     this.childColumns = this.BulkMailTableColumns;
                                // }
                                // 处理字段顺序
                                if (this.currentBox !== 'groupBox' || this.groupBoxId) {
                                    if (typeof showField === 'string') {
                                        const fields = showField.split(',');
                                        this.group = fields.filter((item) => this.editAllColumnsGroup.includes(item));
                                    } else {
            ...
            renderHeader_filter(colIndex) {
                ...
                return (h) => {
                    ...
                    const helpTip = (column.key === 'date' && this.groupBoxId) ? h('HelpTip', {
                        class: '',
                        style: 'margin-left:5px;',
                        props: {
                            title: this.$t('helpTip.mailSendTime')
                        }
                    }) : undefined;
                    return h(
                        'div',
            ...
            renderBulkHeader_filter(colIndex) {
                const column = this.BulkMailTableColumns[colIndex];
                return (h) => {
                    const sortIcons = h(
                        'span',
                        {
                    ...
                    return h(
                        ...
                        [
                            ...
                            h('FilterBox', {
                                ...
                                props: Object.assign(
                                    {
                                        showPopper: this.BulkMailTableColumns[colIndex]
                                            .show_filter_content,
                                        columnCon: this.BulkMailTableColumns[colIndex].key,
                                        order_column: column.key,
                                        columnKey: column.key,
                                        order_type: column.key,
                                        filtering: column.filtering
                                ...
                                on: {
                                    'update:showPopper': (val) => {
                                        this.BulkMailTableColumns = this.BulkMailTableColumns.map((item, index) => {
                                            if (index === colIndex) {
                                                item.show_filter_content = val;
                                            } else if (val) {
                                                item.show_filter_content = false;
                                    ...
                                    'update:query': (val) => {
                                        if (this.config_tableFilter[column.key]) {
                                            if (['sendCount', 'sendedCount', 'sendedRate', 'openCount', 'openRate'].includes(column.key)) {
                                                if (val) {
            ...
            remove_filter_tag(key) {
                ...
                const bulkIndex = this.BulkMailTableColumns.findIndex((item) => item.key === key);
                if (bulkIndex !== -1) {
                    const col = this.BulkMailTableColumns[bulkIndex];
                    col.filtering = false;
                    this.$set(this.BulkMailTableColumns, index, col);
                }
                // 刷新
                this.$store.commit('setCurrentPage', 1);
                this.mailTableData = this.mockmailTableData(this.currentPersonId);
            ...
            change_filter_tag(key) {
                const temp = this.config_tableFilter[key];
                if (temp) {
                    const column = this.mailTableColumns.find((item) => item.key === key) || this.BulkMailTableColumns.find((item) => item.key === key);
                    if (column) {
                        // 判断是否有值, 调整 sqlList
                        const con = key;
                        if (temp.query.length) {
                            ...
                            this.BulkMailTableColumns = this.BulkMailTableColumns.map((item) => {
                                if (item.key === key) {
                                    item.filtering = true;
                                }
                                return item;
                ...
                if (this.currentBox !== 'groupBox' || this.groupBoxId) {
                    for (let i = 0; i < mailList.length; i++) {
                        const data = {
                            ...
                            recipient: mailList[i].mailTo && mailList[i].mailTo[0] === '[' && mailList[i].mailTo[mailList[i].mailTo.length - 1] === ']' ? JSON.parse(mailList[i].mailTo) : (mailList[i].mailTo ? mailList[i].mailTo.split(',') : ''),
                    ...
                    for (let i = 0; i < mailList.length; i++) {
                        const data = {
                            id: mailList[i].groupNo,
                            theme: mailList[i].subject,
                            openCount: mailList[i].openCount,
                            openRate: mailList[i].openRate,
                            sendCount: mailList[i].sendCount,
                            sendedCount: mailList[i].sendedCount,
                            createTime: mailList[i].createTime ? this.dateFormat('yyyy-MM-dd hh:mm:ss', mailList[i].createTime) : '',
                            sendedRate: mailList[i].sendedRate
                        };
                        setData.push(data);
                        /* 此处注意：今天的邮件包含当天以及将来的邮件 */
                        const { day, month, year } = this.get_offset_day(mailList[i].createTime);
                        if (year < 0) {
                            todayArr.push(data);
                        } else if (year > 0) {
                            agoArr.push(data);
                ...
                */
                ...
                if (selecter.type !== 'Range') {
                    ...
                    this.$store.commit(
                        ...
                        GET_MAILBOX_TYPE(item.mailBoxTypeId)
                    );
                    if (
                        this.currentBox === 'draftBox' ||
                        GET_MAILBOX_TYPE(item.mailBoxTypeId) === 'draftBox'
                    ) {
                        if (this.currentPersonId === this.userId) {
                            const obj = {};
                            obj.mailEdit = true;
                            obj.mailId = item.id;
                            obj.selectedMailBoxType = GET_MAILBOX_TYPE(item.mailBoxTypeId);
            ...
            updateStatus(clueId, status) {
                ...
                } else if (this.$refs.tableyesterday && this.$refs.tableyesterday.length) {
                    this.$refs.tableyesterday[0].showClue({ clueId, type: 1 });
                } else if (this.$refs.tableago && this.$refs.tableago.length) {
            ...
            groupBoxHandle(mailId, type) {
                if (type === 'showText') {
                    this.id_previewMail = mailId;
                    this.previewModel = true;
                } else {
                    // this.$store.commit('setCurrentPage', 0);
                    this.$emit('update:groupBoxId', mailId);
                }
            },
            // 回群发箱
            goGroupBox() {
