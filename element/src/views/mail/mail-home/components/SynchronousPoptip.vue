<template>
    <div class="avatar-poptip">
        <Poptip ref="senderPoptip"
                ...
                :open-delay="200"
                placement="top"
                v-show="currentCustomerName || params.row.fromOrToFlag===2"
            ...
            <div class="poptip-content">
                ...
                <div style="display: flex;flex-direction: column;font-size: 12px;" v-if="!requesting">
                    <div class="poptip-content-list poptip-content-contacts">
                        <span class="contactsName">{{crmInfo.type === 7 || crmInfo.type === 8 ? crmInfo.name : crmInfo.contactsName ? crmInfo.contactsName : '-'}}</span>
        ...
        methods: {
                        ...
                        if (res.data.code === 1) {
                            if (res.data.data[email].success) {
                                ...
                                if (res.data.data[this.email].type !== this.crmFlag) {
                                    this.updateCrmFlag(email, res.data.data[email].type);
                                }
            ...
            updateCrmFlag(data, type) {
                util.ajaxMail({
                    ...
                    if (res.data.code === 1) {
                        /**
                         * @Description: 触发changeCrmFlag事件时传入的参数mailCrmFlag错误，导致小人头的颜色和提示语不一致；
                         * 改为recieverCrmFlag
                         * @author 杨娣
                         * @date 2019/9/12
                         */
            ...
            /* 点击小人头对应展示的详情页；我的客户---询盘详情页；公海管理--公海客户详情页；未建档---询盘录入弹窗 */
            operate() {
                /**
                ...
                //         companyId: Number(this.crmInfo.companyId),
                    ...
                    * */
                    ...
                    case 2:
                        this.$emit('showCompany', {
                             ...
                             * @date 2021/01/20
                             */
                            companyId: Number(this.currentInquiryId ? this.currentInquiryId : this.crmInfo.companyId),
                            type: 1,
                            currentMailInfo: currentMailInfo
                        });
                        break;
