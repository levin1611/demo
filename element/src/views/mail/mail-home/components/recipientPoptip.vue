<template>
    <div class="avatar-poptip">
        <Poptip ref="senderPoptip"
                ...
                :open-delay="200"
                placement="top"
                v-show="currentCustomerName || fromFlag === 'tagInput'|| (fromFlag && fromFlag.indexOf('mailDetail') !== -1) || params.row.fromOrToFlag===1"
                popper-class="sender-pop">
            <!--如果currentCustomerName有值，说明是客户箱的列表显示，默认全部显示我的客户的图标-->
            <span slot="reference" @click.stop="operate()" v-if="(currentCustomerName && !fromFlag) || crmFlag===undefined || crmFlag<4">
                <span class="avatar-icon" :style="`color:${avatarColor}; border-color:${avatarColor}`">{{getContactsIdentity(crmFlag)}}</span>
            </span>
            ...
            <div class="poptip-content">
                ...
                <div style="display: flex;flex-direction: column;font-size: 12px;" v-if="!requesting">
                    ...
                    <div class="poptip-content-btns">
                        ...
                        <Dropdown trigger="hover" v-if="crmInfo.type === 0" style="margin-left: 10px;">
                            <Icon
                                size="14"
                                class="el-icon-more"
...
<script>
    ...
    export default {
        props: ['params', 'currentInquiryId', 'currentCustomerName', 'fromFlag', 'currentBoxType', 'currentPage', 'selectedFlag'],
        data() {
            return {
                env,
                requesting: true,
        ...
        computed: {
            ...
            crmFlag() {
                /**
                 * @Description: 此处通过fromFlag和crmInfo.type来限定，当处于写信页面或者详情页时，才从crmInfo中取type作为crmFlag
                 *                 其他情况下沿用原有逻辑。另外因为写信页面和详情页的小人头标识只复用了recipientPoptip.vue组件，因此
                 *                 其他两个组件--senderPoptip.vue和SynchronousPoptip.vue不做修改，特此说明。
                 * @author 杨娣
                 * @date 2019/12/16
                 */
                if (this.fromFlag && this.crmInfo.type) {
                    return this.crmInfo.type;
                } else if (this.params.row) {
                    return this.params.row.recieverCrmFlag;
                }
            },
            avatarColor() {
                // 邮件写信点击邮箱地址选中状态的颜色
                if (this.selectedFlag) {
                    ...
                    if (this.currentCustomerName && !this.fromFlag) {
                        return '#FF752A';
                    }
                    if (this.crmFlag === null || this.crmFlag === undefined || !this.crmFlag) {
                        return '#90949D';
                    } else {
                        switch (this.crmFlag) {
                            case 1:
            ...
            email() {
                if (this.params.row) {
                    return this.getStandardEmail(this.params.row.recipient[0]);
                }
            }
        },
        methods: {
         ...
         return /[a-z0-9]+([.]?[-_a-z0-9]+)*@([a-z0-9]+([.-][a-z0-9]+)*\.)[a-z0-9]+/i.exec(str) ? /[a-z0-9]+([.]?[-_a-z0-9]+)*@([a-z0-9]+([.-][a-z0-9]+)*\.)[a-z0-9]+/i.exec(str)[0] : str;
                    ...
                    util.ajaxJson({
                        ...
                        if (res.data.code === 1) {
                            if (res.data.data[email].success) {
                                if (res.data.data[email].type !== this.crmFlag) {
                                    this.params.row.type = res.data.data[email].type;
                                    this.updateCrmFlag(post_data.emailList[0], res.data.data[email].type);
                                    if (this.fromFlag) {
                                        this.$emit('getAllContactsStatus');
                                    }
                                }
            ...
            operate() {
                ...
                // if (this.currentInquiryId && !this.fromFlag) {
                ...
                switch (this.crmFlag) {
                    ...
                    * */
                    ...
                    default:
                         ...
                         * @date 2020/11/18
                         */
                        /* 当从邮件详情页录入询盘/线索时，中转邮箱的逻辑仅针对发件人有效 */
                        ...
                        if (this.fromFlag && this.fromFlag.indexOf('mailDetail') !== -1 && this.fromFlag !== 'mailDetail-send') {
                            // this.$emit('createCustomer', this.email);
                            this.$emit('createClue', this.params.row.id, this.email, this.fromFlag, currentMailInfo);
                            // return false;
                        } else if (this.fromFlag && this.fromFlag === 'mailDetail-send') {
                            this.$emit(
                                'createClue',
                                this.params.row.id,
                                this.email,
        ...
        watch: {
            params: {
                /* 此监听的目的是：当在邮件详情页录入线索/录入询盘成功后，立即更新小人头的颜色显示。 优化用户体验 */
                // 此监听计算显示邮件小人头颜色功能
                handler(val) {
                    if (val && val.row) {
                        // 重新赋值 crmInfo , 避免计算属性未触发计算, 仍显示上次颜色
                        this.crmInfo = Object.assign({}, this.crmInfo, { type: val.row.recieverCrmFlag });
                    } else {
                        // 数据未请求完成时, 先清空 crmInfo
                        this.crmInfo = {};
                    }
                },
                deep: true
