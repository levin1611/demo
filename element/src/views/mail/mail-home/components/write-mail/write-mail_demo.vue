...
<template>
    <div class="leave_drag" :class="getClass" style="width: 100%;" :style="set_time_flag && set_time_form.local_time ? 'padding-bottom:88px;': 'padding-bottom:56px;'">
                ...
                <Row style="height: 100%;" v-loading="is_show">
                    ...
                    <div class="write-mail-btns-fix" style="background: #fff;width: 99%;padding-left:30px;">
                        ...
                        <div style="display: inline-block;margin-right: 12px;"  v-else>
                            ...
                            <Button type="primary" @click.stop="handleSendEmail" v-show="!sendTip && isSeparate === 1 && publishLoading" style="margin-left: 0;">
                            ...
                            <Dropdown class="send-dropdown" trigger="click" split-button type="primary" placement="bottom-end" @click.stop="handleSendEmail" @command="handle_modal_open" v-show="!sendTip && !publishLoading">
                                {{isSeparate ? $t('mail.writeLetter.respectivelySend') : $t('mail.writeLetter.send')}}
                                <DropdownMenu slot="dropdown" class="mail_label_drop_down">
                                    <DropdownItem command="addtag" >{{$t('mail.set_timeout_mail')}}</DropdownItem>
                                </DropdownMenu>
                            </Dropdown>
                        </div>
        ...
        <Modal
            ...
            <div slot="title" class="dialog-title el-dialog__title">{{$t('mail.separate')}} <a href="http://leadscloud.helpdocsonline.com/untitled-1655552708" target="_blank"><Icon custom="custom custom-tip" size='16' style="margin-left: 4px;"/></a></div>
        ...
        methods: {
            ...
            initEditor() {
                ...
                tinymce.init({
                    ...
                    height: vm.writeType === 'theSameTab' ? vm.window_height - 300 - (vm.set_time_flag && vm.set_time_form.local_time ? 32 : 0) : vm.window_height - 260 - (vm.set_time_flag && vm.set_time_form.local_time ? 32 : 0),
                    language: vm.$t('mail.editor_lauguage'),
                    plugins: ['undo', 'redo', 'fullscreen', 'autolink', 'lists', 'link', 'image', 'charmap', 'print', 'hr', 'anchor', 'imagetools', 'media', 'nonbreaking', 'directionality', 'textpattern', 'formatpainter', 'table', 'hr', 'code', 'emoticons'],
                    // 添加扩展插件
                    external_plugins: {
                    ...
                    toolbar: [
                        'undo redo formatpainter | fontselect fontsizeselect | forecolor backcolor bold italic underline strikethrough | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent lineheight | image link table emoticons hr | fullscreen code removeformat'
                    ],
                    ...
                    images_upload_handler: function(blobInfo, success, failure) {
                        ...
                        // vm.imagesUpload(blobInfo, success, failure);
                        const blobInfoList = vm.blobInfoList.findIndex(item => blobInfo.filename() === item.filename());
                        if (blobInfoList >= 0) {
                            vm.blobInfoList.splice(blobInfoList, 1, blobInfo);
                        } else {
                            vm.blobInfoList.push(blobInfo);
                        }
            ...
            imagesUpload() {
                ...
                let editorBody = this.currEditor.getBody();
                const imgSrc = editorBody.getElementsByTagName('img');
                util.ajax({
                    url: '/file-sys/api/uptoken',
                    method: 'post'
                }).then(async response => {
                    ...
                    this.blobInfoList.forEach(blobInfo => {
                        console.log(blobInfo)
                        const uid = Date.now() + blobInfo.name();
                        const observable = qiniu.upload(blobInfo.blob(), uid, this.uptoken, putExtra, config);
                        observable.subscribe(() => {
                            ...
                            for (let i = 0; i < imgSrc.length; i++) {
                                if (imgSrc[i].src === blobInfo.blobUri()) {
                                    imgSrc[i].setAttribute('src', this.qiniuHttp + res.key);
                                }
                            }
                            // success(`https://content.leadscloud.com/${res.key}`);
                        });
                    });
                });
                editorBody = this.currEditor.getBody();
            },
            /** 获取写信设置默认值 */
            getDefaultSetting() {
                util.ajaxJson({
         ...
         return /[a-z0-9]+([.]?[-_a-z0-9]+)*@([a-z0-9]+([.-][a-z0-9]+)*\.)[a-z0-9]+/i.exec(str) ? /[a-z0-9]+([.]?[-_a-z0-9]+)*@([a-z0-9]+([.-][a-z0-9]+)*\.)[a-z0-9]+/i.exec(str)[0] : str;
            ...
            getMailAccountsSelectForGroup(type) {
                ...
                writeApi.mailAccountsSelectForGroup({
                    createUserId: this.userId,
                    orgId: this.enterpriseId,
                    groupDate: this.set_time_form.local_time ? this.$options.filters.timeFormat(this.bj_time) : this.$options.filters.timeFormat(new Date())
                }).then(res => {
                    if (res.code === 1) {
            ...
            paramVerify() {
                ...
                return new Promise((resolve) => {
                    writeApi.paramVerify({
                        ...
                        bjTime: this.set_time_form.local_time ? (this.bj_time_flag ? this.bj_time_flag : new Date(this.bjTime).getTime()) : undefined
                    }).then(res => {
                        if (res.code === 2) {
