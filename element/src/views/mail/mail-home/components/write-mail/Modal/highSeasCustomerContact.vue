<template>
    <div class="contacts_content_conatiner">
        ...
        <Form label-width="80px" :model="contactsFormForShow" class="customer-contact">
            <Row :gutter="40" style="margin: 0;">
                <Col :span="(index+1) % 2 === 1 ? 13 : 11" style="padding:0;" v-for="(item, index) in filterFieldList" :key="index">
                    <FormItem :label="item.label || item.fieldName">
                        ...
                        <!-- 多选 重点标记 -->
                        <CheckboxGroup
                                v-else-if="item.fieldType === 'selectMultiple' && item.field === 'marker' && contactsFormForShow[(filterFieldsModule === 1 ? 'cp' : 'iq') + $dym.firstUpperCase(item.field)]"
                                v-model="contactsFormForShow[(filterFieldsModule === 1 ? 'cp' : 'iq') + $dym.firstUpperCase(item.field)]">
                        ...
                        <Select v-model="contactsFormForShow[item.field]"
                                v-else-if="item.fieldType === 'selectMultiple' && item.field === 'companyManagersBefore'"
                                multiple
                                filterable
                                :disabled="filtering"
                                clearable
                        ...
                        <!-- 多选 非负责人、非重点标记、非主联系人 -->
                        <Select v-model="contactsFormForShow[(filterFieldsModule === 1 ? 'cp' : 'iq') + $dym.firstUpperCase(item.field)]"
                                v-else-if="item.fieldType === 'selectMultiple' && item.field !== 'companyManagersBefore' && item.field !== 'marker' && item.field !== 'linkman'"
                                multiple
                                filterable
                                :disabled="filtering"
                                clearable
            ...
            <FormItem>
                <Button @click="searchContact" type="success" :loading="searchLoading" :class="`${filterFieldsModule === 1 ? 'piwik_high_seas_customer_contact_search_btn' : 'piwik_high_seas_inquiry_contact_search_btn'}`">{{$t('mail.writeLetter.addContactModal.filter')}}</Button>
        ...
        data() {
            return {
                ...
                lang_clue: {
                    ...
                    giveupReason: this.$t('crm.Table.col_lastGiveupReason'),
                    // 询盘
        ...
        methods: {
            ...
            setDataFormat(obj) {
                ...
                this.filterFieldList.forEach((item) => {
                    ...
                    // 关键字、多选需要$or,其他字段均需要push到$and数据即可, 排除前负责人 companyManagersBefore 字段，不走字符串拼接 query 字段查询
                    if (item.field !== 'subUserIds' && item.field !== 'linkman') {
                        if (item.fieldType === 'selectMultiple' && item.field !== 'companyManagersBefore' && item.field !== 'inquiryScore') {
                            let symbolName;
            ...
            getContactsEmailCompanyNameList(callBackType) {
                ...
                const dataObj = {
                    ...
                    companyManagersBefore: this.contactsFormForShow.companyManagersBefore && this.contactsFormForShow.companyManagersBefore.join(',') // 前负责人单独传
                };
                ...
                util.ajax({
                    url: '/crm/mailbox/getPublicSeaCompanyContactsNameList',
                    method: 'post',
                    data: dataObj
                }).then(response => {
                    ...
                    if (this.hasMailAuthority && this.isContainsMenu('highseasCustomer')) {
            ...
            filterDataInfo() {
                ...
                for (let i = 0; i < this.filterFieldList.length; i++) {
                    ...
                    } else if ((item.fieldType === 'selectMultiple' && item.field !== 'companyManagersBefore') || item.fieldType === 'date') {
                        // 排除前负责人 companyManagersBefore 字段，不走字符串拼接 query 字段查询
                        this.$set(this.contactsFormForShow, [(this.filterFieldsModule === 1 ? 'cp' : 'iq') + this.$dym.firstUpperCase(item.field)], []);
                    } else if (item.field === 'companyManagersBefore') {
                        // 前负责人字段单独处理
