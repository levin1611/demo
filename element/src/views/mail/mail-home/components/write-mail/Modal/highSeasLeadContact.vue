<template>
    <Form label-width="80px" :model="contactsFormForShow" class="clue-contact contacts_content_conatiner">
        ...
        <FormItem>
            <Button @click="searchContact" type="success" :loading="searchLoading" class="piwik_high_seas_lead_contact_search_btn">{{$t('mail.writeLetter.addContactModal.filter')}}</Button>
        ...
        methods: {
            ...
            async searchContact() {
                ...
                this.filterFieldList.forEach((item) => {
                    ...
                    // 关键字、多选需要$or,其他字段均需要push到$and数据即可
            ...
            getColumnValueList(callBackType) {
                ...
                */
                ...
                util.ajax({
                    url: '/form-cust/form/getPublicSeaColumnValueList',
                    method: 'post',
                    data: sendData
                }).then(response => {
                    ...
                    */
                    if (response.data.code) {
                        ...
                        if (response.data.data && this.hasMail && this.isContainsMenu('common_sea_customer')) {
