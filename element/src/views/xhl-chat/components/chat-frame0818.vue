    ...
    <div class="chat-frame-container">
        ...
        <div v-if="source==='FullScreen'" class="frame-top">
            ...
            <div class="frame-top-item" style="display: flex;align-items: center;float: right;margin-right: 10px;margin-top: 13px;">
                <div class="icon-list" @click="$emit('toggleShowLittleDrawer')" style="cursor: pointer">
                    <Tooltip v-if="visitorBaseInfo.crmStatus === '2'"
                         :content="$t('clue.clue')"
                         placement="left">
                        <Icon custom="custom-clue" size="32"></Icon>
                    </Tooltip>
                    <Tooltip v-else-if="visitorBaseInfo.crmStatus === '102'"
                         :content="$t('clue.inquiry')"
                         placement="left">
                        <Icon custom="custom-inquiry" size="32"></Icon>
                    </Tooltip>
                    <Tooltip v-else
                         :content="visitorBaseInfo.crmStatus === '3' ? $t('littleDrawer.irrelevantCustomer') : $t('littleDrawer.undocumentedCustomers')"
                         placement="left">
                        <Icon custom="custom-unfiled" size="32"></Icon>
                    </Tooltip>

                </div>
                <!-- <Dropdown v-if="statusFlag != '2' && statusFlag != '102'"
                          trigger="click"
                ...
                <span v-show="statusFlag !=='3'">
                    <Icon class="clue-btn" v-if="statusFlag=='1'" color="#FA8241" custom="custom custom-enter-clue"
                          size="24" @click="changeModel"/>
                    <Tooltip :content="currVisitorType === 'crmType' ? $t('clue.inquiry') : $t('clue.clue')"
                        ...
                        <svg class="svg-icon" aria-hidden="true" @click="showCrm()"
                            ...
                            <use v-if="currVisitorType === 'clueType'" xlink:href="#custom-clue"></use>
                            <use v-else-if="currVisitorType === 'crmType'" xlink:href="#custom-inquiry"></use>
                        </svg>
                    </Tooltip>
                </span> -->
            </div>
        ...
        <div class="frame-footer">
            ...
            <msg-send :to="currSessionId" :parentView="source" @letShortcutsShow="shortCutsShow"></msg-send>
        </div>
        ...
        <!-- <clueModal v-model="inputClue"
                   ...
                   :saleList="customerServices"
                   :templateList="cluefields"
        ...
        methods: {
            ...
            async changeStatus(val) {
                ...
                if (ret) {
                    util.ajaxInternationalJson({
                        ...
                        if (res.data.code === '1') {
                            this.$emit('updateVisitorInfo', Object.assign(this.visitorBaseInfo, {
                                crmStatus: val
                            }));
            ...
            getSaleList() {
                util.ajax({
                    ...
                    if (res.data.code === '1') {
                        this.customerServices = res.data.data;
