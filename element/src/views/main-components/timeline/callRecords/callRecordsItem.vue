<template>
    <div class="timeline-item twemoji-convert">
        <div class="item-header clearfloat">
            <!-- 标题 -->
            <div class="item-title">{{ `${item.callType}` === '0' ? $t('timeLine.callIn') : $t('timeLine.callOut') }}</div>
        ...
        <div class="item-body">
            ...
            <!-- "我"发送的消息 -->
            <span v-if="item.callType === 0">
                {{ $lang === 'zh-CN' ?
                    `【${item.contactsName}】${$t('facebookInteraction.to')}【${item.eventUser}】${$t('timeLine.callTitle')}`
                    :
                    `【${item.contactsName}】${$t('facebookInteraction.to')}【${item.eventUser}】${$t('timeLine.callTitle')}`
                }}
            </span>
            <span v-if="item.callType === 1">
                {{ $lang === 'zh-CN' ?
                    `【${item.eventUser}】${$t('facebookInteraction.to')}【${item.contactsName}】${$t('timeLine.callTitle')}`
                    :
                    `【${item.eventUser}】${$t('facebookInteraction.to')}【${item.contactsName}】${$t('timeLine.callTitle')}`
                }}
            </span>
            ...
            <!-- <Poptip width="500"
                ...
                <span @click="handleOpenWhatsAppModal(item)"
                      :class="`item-event ${class_whatsApp_GTM}`">{{ $t('timeLine.callDetails') }}</span>
        ...
        <!-- <template v-if="JSON.parse(item.eventCustomVar).type === 1">
            <div v-if="JSON.parse(item.eventCustomVar).qwText" class="whatsapp-text">{{`${$t('mail.writeLetter.body')}： ${JSON.parse(item.eventCustomVar).qwText}`}}</div>
        </template>
        <template v-else>
...
<script>
    ...
    import qwViewMsg from '@/views/main-components/timeline/Micronent/qwViewMsg.vue';
    import timeline from '@/api/timeline';
    const { getWaMessageId } = timeline;

    export default {
        name: 'callRecordsItem',
        ...
        methods: {
            ...
            async handleOpenWhatsAppModal(item) {
                this.$EVENTBUS.$emit('openCalldetailIng', true, {
                    userId: JSON.parse(item.eventCustomVar).bindCinnoxUserId,
                    orgId: this.orgId,
                    messageId: JSON.parse(item.eventCustomVar).messageId,
                    staffPhone: JSON.parse(item.eventCustomVar).userBindPhone
                });
                ...
                // this.whetherToLocateTheMessage = JSON.parse(item.eventCustomVar).messageId;
                // this.chatObj = {
                //     robotWxid: JSON.parse(item.eventCustomVar).userBindQwAccount,
                //     fromQwAccount: JSON.parse(item.eventCustomVar).fromQwAccount,
                //     toQwAccount: JSON.parse(item.eventCustomVar).toQwAccount,
                //     fromMe: JSON.parse(item.eventCustomVar).fromMe,
                //     bindUserId: JSON.parse(item.eventCustomVar).bindQwUserId
                // }
                ...
                //     this.chatQwAccount = JSON.parse(item.eventCustomVar).toQwAccount;
                // } else {
                //     // 聊天方为私人
                ...
                //         this.chatQwAccount = JSON.parse(item.eventCustomVar).fromQwAccount;
                ...
                // this.userBindQwAccount = JSON.parse(item.eventCustomVar).userBindQwAccount;
