    ...
    <div class="leave_drag" style="width: 100%;" :style="set_time_flag && set_time_form.local_time && isShowOnSeparate ? (isModalFullScreen ? 'padding-bottom:24px;': 'padding-bottom:64px;'): (isModalFullScreen ? 'padding-bottom:0;': 'padding-bottom:30px;')">
        <div style="height: 100%;" class="writeMail writeMailModal ">
            <div @click.stop="writeMail" @paste="handlePaste">
                ...
                <Row style="height: 100%;">
                    ...
                    <Card shadow="never" :bordered="false" dis-hover v-if="!isSendMailSuccess" v-loading="is_show">
                        <Row :gutter="20">
                            ...
                            <div class="enter_drag">
                                <!--<span class="separate mail-massmailing" v-if="isSeparate === 0 && writeType==='theSameTab'"  @click="handleSeparate">{{$t('mail.separate')}}</span>-->
                                <!--<span class="separateInNewTab mail-massmailing" v-if="isSeparate === 0 && writeType!=='theSameTab'" @click="handleSeparate">{{$t('mail.separate')}}</span>-->
                                <!--<span class="separate" v-if="isSeparate === 1 && writeType==='theSameTab'" @click="handleSeparate">{{$t('mail.cancelSeparate')}}</span>-->
                                <!--<span class="separateInNewTab" v-if="isSeparate === 1 && writeType!=='theSameTab'" @click="handleSeparate">{{$t('mail.cancelSeparate')}}</span>-->
                                ...
                                <!-- 【展开】最近联系人按钮 -->
                                <!--<i style="position:absolute;top:50%;margin-top:-20px;right:-42px;">-->
                                    <!--<Tooltip placement="top" transfer v-if="!showContactList&&!textModel&&!translateModel" :content="$t('distribution.open')">-->
                                        <!--<svg class="svg-icon custom"  v-if="!showContactList&&!textModel&&!translateModel" @click="openContact" aria-hidden="true" style="display: inline-block;font-size:50px;cursor:pointer;">-->
                                ...
                                <!--<i style="position:absolute;top:50%;margin-top:-20px;right:-42px;">-->
                                    <!--<Tooltip placement="top" transfer v-if="showContactList&&!textModel&&!translateModel" :content="$t('distribution.takeUp')">-->
                                        <!--<svg class="svg-icon custom" v-if="showContactList&&!textModel&&!translateModel" @click="takeUpContact" aria-hidden="true" style="display: inline-block;font-size:50px;cursor:pointer;">-->
                                ...
                                <Form  v-model="emailForm">
                                    ...
                                    <FormItem style="margin-bottom: 10px;">
                                        <div  class="form-title-font write-mail-tags" style="border-bottom:1px solid #EAEAEA;margin-bottom: 0">
                                    ...
                                    <FormItem class="selfUpload-formItem">
                                        <div @click.stop style="display:inline-block;">
                                            ...
                                            <!-- 翻译icon图标 -->
                                            <span class="blue-font" v-if="!dragOver" style="cursor: pointer;display: inline-block;position: absolute;top:0;left: 300px; height: 30px;line-height: 26px;margin-top:1px;">
                                            ...
                                            <!-- 快速文本 -->
                                            <span class="blue-font" v-if="!dragOver" @click.stop="quickTextClick" style="cursor: pointer;display: inline-block;position: absolute;top:0;right: 0; height: 30px;line-height: 30px;margin-top:1px;">{{$t('mail.writeLetter.fastText')}}</span>
                                        </div>
                                        <!-- 监测链接的入口 -->
                                ...
                                <div class="write-mail-btns-fix" style="position: absolute;bottom:0;left:0;padding: 10px 20px 24px 20px; width: 100%; background: #fff;">
                                    ...
                                    <div style="display: inline-block;">
                                        <!-- 签名 -->
                                        <Dropdown trigger="click" style="margin-left: 20px;" placement="bottom-end" @command="handleSignChange">
                    ...
                    <!--<ul class="linkman-list"-->
                        ...
                        <!--:style="`max-height:${writeType!=='theSameTab'?windowHeight+60:(window_height - 50)}px;`">-->
                        <!--<li>-->
                        <!--<li style="cursor:auto;">-->
                            <!--{{$t('letters.linkmanLately')}}-->
                        <!--</li>-->
                        <!--<li v-for="(linkman,index) in linkman_list" class="mail-recentcontact"  @click="handle_link_man(linkman)" :key="index">-->
                            <!--{{linkman.mailRecipientAddress}}-->
                        <!--</li>-->
                    <!--</ul>-->
                    <!--</Col>-->
                    <!--
            ...
            <Modal :title="$t('mail.writeLetter.preview')"
                ...
                <blockquote class="detailContent" v-html="mailContent" style="overflow-x: auto;padding-top:20px;line-height:32px;" >
                </blockquote>
                <div slot="footer">
...
<script>
    ...
    import AddContact from '../../../mail/mail-home/components/write-mail/Modal/address_book_modal';
    import UploadList from '../../../mail/mail-home/components/write-mail/upload-list';
    import selfUpload from '../../../mail/mail-home/components/write-mail/self-upload';
    ...
    export default {
        ...
        created() {
            ...
            this.getAccountsByUserId(); /* 通过用户ID获取发件人邮箱列表 */
            // this.getTextsByUserId(); /* 通过用户ID获取快速文本列表 */
            // this.get_linkman_list(); // 获取最近联系人
            // this.searchFastTextList();  // 搜索快速文本
            this.$store.dispatch('getCRMFieldAPI'); /** 请求crm输入上限 */
            this.getTags();
            /** 获取写信默认设置 */
        ...
        mounted() {
            ...
            this.$nextTick(() => {
                ...
                setTimeout(() => {
                    ...
                    const iframe = document.getElementById('vModalEditor_ifr');
        ...
        methods: {
            ...
            addDataBaseFile(arr) {
                // 获取邮件富文本的dom
                const currEditor = tinymce.editors.find(item => item.id === 'vModalEditor');
            ...
            initPage() {
                            ...
                            if (response.data.code === 1 || response.data.code === -4) {
                                ...
                                if (type === 'draftBox') {
                                    ...
                                    if (!data.fromOrToFlag) {
                                        ...
                                        /* 发件 */
                                        if (rType === 'replyAll' || rType === 'replyCurrent' || rType === 'replyWithAttachment') {
                                            // 当前邮件的发件人等于原发件人，即为
                                            ...
                                            if (((rType === 'replyAll' || rType === 'replyWithAttachment') && isRecipients_to) || rType === 'replyCurrent') {
                                            ...
                                            if (!isRecipients_to && (rType === 'replyAll' || rType === 'replyWithAttachment')) {
                ...
                if (sendMailContents) {
                    ...
                    let fieldsNode = '';
                    fields.forEach(item => {
                        fieldsNode += `<p style="margin:0;padding-left:15px;line-height:24px;">${item.label}: ${item.value}</p>`;
                    });
                    const mailContent = `<div id="relayPlaceholder" style="line-height: 32px;font-size:14px ;font-family: arial,helvetica,sans-serif;color:#333;">
                                    -------- Reply message --------<br>
                    ...
                    setTimeout(() => {
                        vm.currEditor.resetContent(mailContent);
            ...
            getAccountsByUserId() {
                ...
                let params;
                if (['replyCurrent', 'replyAll', 'replyWithAttachment'].includes(this.replyMailType) && this.mailDetailId) {
                    params = {
                        originalMailId: this.mailDetailId,
            ...
            handleSignChange(index) {
                ...
                if (this.signatures[index].id === '-1') {
                    ...
                    signatureNew = `<div id="signature" style="color:#000;font-size:16px;font-family:楷体,arial,helvetica,sans-serif;clear:both;">${
                        this.signatures[index].signContent
                    }</div>`;
                    signature = `<div id="signature" style="color:#000;font-size:16px;font-family:楷体,arial,helvetica,sans-serif;clear:both;">${
                        this.signatures[index].signContent
                    }</div>`;
                }
                    ...
                    util.ajaxJson({
                     ...
                     同时删除邮件数据库的原草稿邮件 */
                            ...
                            if (this.taskObj && this.isSeparate === 0) {
                                ...
                                this.taskObj.relatObjDescription = JSON.stringify({ mailSubject: data.subject, mailId: response.data.data, mailBoxTypeId: 5, mailBoxTypeName: this.$t('mail.draftBox'), mailCreateUserId: this.userId });
