  ...
  <div class="tag-input" @click="setFocus">
    <div slot="prefix" class="tag-list">
        <Tag :color="tagAttr[index].isEmail?'#f7f7f7':'#ed4014'" v-for="(tag,index) in tagList" :key="tag.emailAddress">
            <span class="tag-content" :title="tag.tagName"  @dblclick="toggleEdit(index,true)">{{tag.tagName}}</span>
            <input :id="type+'input'+index"
                v-model="tagList[index].tagName"
                v-show="tagAttr[index].editting"
                @keyup.enter="editCompleted(index)"
                @blur.stop="editCompleted(index)"
                :style="!tagAttr[index].isEmail && 'border-color: #ed4014;'"
                class="input-inside input-inside-tag" type="text">
    ...
    <div>
        <Input
                ...
                @paste.native.capture.prevent="handlePaste"></Input>
...
.tag-input{
    ...
    .tag-list{
        ...
        /deep/ .el-tag {
            ...
            .mail-deleteemailaddress{
                display: none;
            }
            &:hover .mail-deleteemailaddress{
                display: inline-block;
            }
        }
        .tag-content{
