...
export function getHorizonScrollBarSize(fresh) {
    if (isServer) return 0;
    if (fresh || cached === undefined) {
        const inner = document.createElement('div');
        ...
        outer.style.overflow = 'scroll';
        let heightScroll = inner.offsetHeight;

        if (heightContained === heightScroll) {
            heightScroll = outer.clientHeight;
        }

        document.body.removeChild(outer);

        cached = heightContained - heightScroll;
    }
    return cached;
}
