        ...
        methods: {
            ...
            getColumnList() {
                ...
                util.ajax({
                    ...
                    if (response.data.code == 1) {
                        let columnList = [
                            // {title: this.$t('website.index'), type: 'index', align:'center', width: 60, fixed: 'left', key:''},
                            { title: this.$t('website.proj'), key: 'proj', align: 'center', width: 200 }
                        ];
                        const groupTemp = [this.$t('website.proj')];
                        ...
                        columnList = columnList.concat([
                            { title: this.$t('website.currentUrl'), minWidth: 200, align: 'center', key: 'currentUrl' },
                            { title: this.$t('website.sourceUrl'), minWidth: 200, align: 'center', key: 'sourceUrl' },
                        ...
                        this.allcolumns = columnList.concat();
                        this.conditionlist1 = groupTemp.concat([this.$t('website.currentUrl'), this.$t('website.sourceUrl'), this.$t('website.remark'), this.$t('website.operate')]);
            ...
            changecon1() {
                ...
                switch (this.condition1) {
                    // 时间类
                    case this.$t('website.createTime'):
            ...
            getSearchCon(str) {
                for (const item of this.allcolumns) {
                    if (str === item.title) {
                        if (str == this.$t('website.currentUrl')) {
                            return 'currentUrl';
                        }
                        if (str == this.$t('website.createTime')) {
                            return 'createTime';
                        }
                        if (str == this.$t('website.sourceUrl')) {
                            return 'sourceUrl';
                        }
                        if (str == this.$t('website.operate')) {
                            return 'status';
                        } else if (item.key.indexOf('ba10') !== -1) {
