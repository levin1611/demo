<template>
    <div class="crm-table-page">
                    ...
                    <template v-if="visible_batch_operate">
                        <div style="display: inline-block;color: rgba(0, 0, 0, .8);margin-bottom: -15px;"
                            ...
                            <Button v-if="authorized_button['mergeData']"
                                    type="minor"
                                    style="font-size: 14px; padding: 10px;margin-left: 20px;"
                                    @click="merge_inquiry">{{ $t('crm.Table.merge') }}</Button>

                            <!-- 批量转移询盘 -->
                            <!--<Button v-if="authorized_button['transferInquiryBatch']"-->
                            <!--        type="text"-->
                            <!--        icon="custom custom-crm-transfer"-->
                            <!--        @click="transfer_inquiry">{{ $t('crm.Detail.transferInquiry') }}</Button>-->

                            <!-- 受众优化 -->
                            <Button v-if="authorized_button['audienceOptimization']"
                                    type="minor"
                                    style="font-size: 14px; padding: 10px;margin-left: 10px;"
        ...
        <!-- 批量转移询盘 -->
        <!--<TransferInquiryBatch :visible.sync="visible_transfer_inquiry"-->
        <!--                      :inquiryIds="selected_items.map(item => item.inquiryId)"-->
        <!--                      :clueIds="selected_items.map(item => item.clueId)"-->
        <!--                      :inquiryManagers="selected_items.map(item => item.inquiryManagerIds)"-->
        <!--                      :inquiryManagerNames="selected_items.map(item => item.inquiryManagers)"-->
        <!--                      @refreshData="refreshData({clearSelected: true})"></TransferInquiryBatch>-->
...
<script>
    ...
    import MergeData from '@/views/crm/Table/Components/Modal/mergeData';
    ...
    // import TransferInquiryBatch from '@/views/crm/Table/Components/Modal/transferInquiryBatch';
    import AudienceOptimization from '@/views/crm/Table/Components/Modal/audienceOptimization';
        ...
        switch (this.addCondition[index].condition1) {
            ...
            case 'inquiryManagers':
                this.addCondition[index].conditionList2 = [
                    {
                        value: '$eq',
                        label: this.$t('crm.Table.con_include') // this.$t('highSeasSetting.containsAny') this.$t('crm.Table.con_equal')
                    }
                ];
                this.addCondition[index].condition2 = '$eq';
                this.addCondition[index].inputType = 'array';
    ...
    export default {
        ...
        computed: {
            ...
            authorized_button() {
                ...
                return {
                    ...
                    // transferInquiryBatch: !HANDLE_BUTTON(this.button_ids.transferInquiryBatch[env], this.button_list)
                };
            },
            ...
            computed_sqlList_tableFilter() {
                return Object.entries(this.sqlList_tableFilter).map(([key, val]) => {
                    ...
                    } else if (['transactionMoney', 'follow_up_count', 'not_followed_days', 'estimated_inquiry_amount'].includes(key)) {
                        temp.$and = val.$and;
        ...
        data() {
            return {
                ...
                lang_inquiry: {
                    ...
                    follow_up_count: this.$t('crm.Table.col_followUpCount'),
                ...
                // 转移
                // visible_transfer_inquiry: false,
        ...
        methods: {
                ...
                if (localStorage.getItem(getLocalStorageItemName.workBench_number_type)) {
                    ...
                    switch (temp) {
                        ...
                        case 'unFollow':
                            ...
                            this.addClick();
                            // this.addCondition[0].condition1 = 'updateTime';
                            // this.addCondition[0].condition2 = '$lt';
                            // this.addCondition[0].condition3 = start.format('yyyy-MM-dd');
                            // this.addCondition[0].inputType = 'date';
            ...
            /**
             * @Description: 注释掉自定义 checkbox 相关方法, 保险起见先不删掉
             * @author 汤一飞
             * @date 2020/4/30
             */
            // checkSingle(val, scope) {
            //     if (val) {
            //         const { inquiryId, companyId, clueId, inquiryManagers, inquiryManagerIds } = scope.row;
            ...
            //         });
            //     } else {
            //         this.selected_items = this.selected_items.filter(item => item.inquiryId !== scope.row.inquiryId);
            //     }
            // },
            // 清除表格已选内容
            clear_selected_items() {
                ...
                if (this.$refs.customerView.$refs['table']) {
                    this.$refs.customerView.$refs['table'].clearSelection();
                }
                this.selected_items = [];
            },
            ...
            /* 批量转移询盘 */
            // transfer_inquiry() {
            //     this.visible_transfer_inquiry = true;
            // },
            ...
            /* 文件下载 */
            exportData() {
                window.open(`${this.$util.ajaxUrl}/crm/mongo/inquiryRefactor/getFilterListExcel?userId=${this.userId}&orgId=${this.enterpriseId}&listType=3&query=${this.queryData.$and.length ? encodeURIComponent(JSON.stringify(this.queryData)) : ''}&subUserIds=${this.subUserIds}&isFromGoogle=${this.isFromGoogle ? 1 : ''}&optimizeStatus=${this.optimizeStatus || ''}&auditStatus=${this.auditStatus || ''}`, '_blank');
            },
                ...
                */
                ...
                crmInquiry.getInquiryFilterListDataAndTotalMongo(postData, cancelToken).then(async response => {
                    if (response.code === '1') {
                        ...
                        // 表格滚动到顶部
                        if (this.$refs.customerView.$refs['table']) {
                            this.$refs.customerView.$refs['table'].$el.querySelector('.el-table__body-wrapper').scrollTop = 0;
                        }
                    } else {
                        this.totalNum = 0;
                        this.maxPageNum = 0;
                ...
                if (Array.isArray(data) && data.length) {
                    ...
                    data = data.map(data => {
                        const temp = {
                            ...
                            transactionTime: data.transactionTime ? new Date(data.transactionTime).format('yyyy-MM-dd hh:mm') : '',
                            transactionMoney: (data.transactionMoneyCurrency && data.transactionMoney != null) ? (Utils.getCurrencySymbol(data.transactionMoneyCurrency).symbol + Utils.transformMoney(data.transactionMoney)) : '-',
            ...
            renderHeader_filter(colIndex) {
                return h => {
                    ...
                    // 系统 tips
                    const helpTip = ['lastFollowupTime', 'updateTime', 'follow_up_count'].includes(column.key)
                        ? h('HelpTip', {
                            ...
                            props: {
                                title: (key => {
                                    switch (key) {
                                        case 'lastFollowupTime':
                                            return this.$t('helpTip.lastFollowupTime');
                                        case 'updateTime':
                                            return this.$t('helpTip.updateTime');
                                        case 'follow_up_count':
                                            return this.$t('helpTip.follow_up_count');
                                    }
                                })(column.key)
                            }
                        })
                        ...
                        on: {
                            ...
                            'update:query': (val) => {
                                if (this.config_tableFilter[column.key]) {
                                    if (['transactionMoney', 'follow_up_count', 'notFollowedDays', 'estimatedInquiryAmount'].includes(column.key)) {
                                        if (val) {
                    ...
                    }, [title, (this.order_column === order_column && this.order_type) ? sortIcons : undefined, column.hasOwnProperty('show_filter_content') ? filterPop : undefined, helpTip]);
                };
            },
            ...
            //     // return h('span', ' ');
            //     const ids_selected = this.selected_items.map(item => item.inquiryId);
            //     return h('Checkbox', {
            //         props: {
            //             value: this.tableData.every(item => ids_selected.includes(item.inquiryId))
            //         },
            //         on: {
            //             change: (val) => {
            //                 if (val) {
            ...
            //                         if (!ids_selected.includes(item.inquiryId)) {
            //                             const { inquiryId, companyId, clueId, inquiryManagers, inquiryManagerIds } = item;
            //                             temp.push({
            //                                 inquiryId,
            //                                 companyId,
            ...
            //                         }
            //                     });
            //                     this.selected_items = this.selected_items.concat(temp);
            //                 } else {
            //                     this.selected_items = this.selected_items.filter(item => !ids_selected.includes(item.inquiryId));
            //                 }
            //             }
            //         }
            //     });
            ...
            clickRow(data, column, event) {
                ...
                */
                if (selector.type !== 'Range' &&
                    ...
                    this.show_inquiry_detail = true;
                    this.rowIndex = this.tableData.findIndex(item => item.inquiryId === data.inquiryId);
                }
            },
            ...
            headerCellClass({ column }) {
                ...
                if (['lastFollowupTime', 'updateTime', 'follow_up_count'].includes(column.property)) {
