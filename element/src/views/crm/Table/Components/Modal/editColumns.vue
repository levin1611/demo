<template>
    <Modal :visible="visible"
           ...
           :modal-append-to-body="modalAppendToBody ? modalAppendToBody : false"
           :append-to-body="modalAppendToBody ? modalAppendToBody : false"
        ...
        <div slot="title">
            <span style="font-weight: 700;font-size: 16x;color: rgba(0,0,0,0.80);letter-spacing: 0;line-height: 26px;">
                {{ title ? title : $t('crm.Table.title_editColumns') }}
            </span>
        ...
        <div style="margin: 20px 0 6px;position: relative;">
            ...
            <Transfer :data="col_all"
                      ...
                      @hook:mounted="getTransferHeight"></Transfer>
            <!-- 上下移动字段 -->
...
<script>
    ...
    export default {
        ...
        props: {
            ...
            contactFilterPoptipVisible: Boolean,
            fieldDisableAndFixed: String
        },
        computed: {
            ...mapState({
        ...
        methods: {
            ...
            toggle_visible(targetKeys, direction, moveKeys) {
                ...
                if (this.fieldDisableAndFixed) {
                    const companyNameIndex = targetKeys.findIndex(item => item === this.fieldDisableAndFixed);
                    if (companyNameIndex !== -1) {
                        targetKeys.splice(companyNameIndex, 1);
                    }
                    targetKeys.unshift(this.fieldDisableAndFixed);
                }
                ...
                switch (direction) {
                    case 'up':
                        this.col_selected_show.forEach(item => {
                             ...
                             * @date 2021/6/9
                            */
                            if (index > (this.columnType === 1 || this.fieldDisableAndFixed ? 1 : 0) && !this.col_selected_show.includes(this.col_show[index - 1])) {
            ...
            // 关闭弹框回调,控制通讯录气泡弹框保持弹出状态
            closeModal() {
                if (this.contactFilterPoptipVisible) {
                    setTimeout(() => {
                        this.$emit('update:contactFilterPoptipVisible', true);
                    });
                }
            }
        },
                    ...
                    if (val) {
                        this.col_all = this.allGroup.map(item => {
                            return {
                                ...
                                parentId: [1, 2, 3, 4].includes(this.columnType) ? this.belongMap[item] : undefined,
                                disabled: (this.columnType === 1 && item === 'cpCompanyName') || item === this.fieldDisableAndFixed
                            };
                        });
