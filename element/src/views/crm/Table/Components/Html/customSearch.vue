<template>
                ...
                <div v-for="(add, index) in addCondition" class="conditionmain" :key="index">
                    ...
                    <Select v-model="add.condition1" style="width:120px;margin-right:15px;float:left;"
                        ...
                        <Tabs v-model="fieldBelong" class="custom-tabs-class">
                            <TabPane :label="$t('whatsapp_manage.conversation')" name="1">
                            ...
                            <template v-if="type != 'whatsapp' "> 
                                <!-- whatsapp模块调用只有会话tab -->
                            <TabPane :label="$t('crm.Table.customer')" name="2">
                                <Option v-for="(item, index) in add.conditionList1.customerField" :value="item.value" :label="item.label"
                                        :key="`${columnType}customer${index}`">{{item.label}}
                                </Option>
                            </TabPane>
                            <TabPane :label="$t('crm.Table.inquiry')" name="3" style="padding-right: 20px;">
                            ...
                            <TabPane :label="$t('crm.Html.contact')" name="4">
    ...
    <!--                                {{ n === 1 ? $t('crm.Table.con_is') : n === 2 ? $t('crm.Table.con_not') : '' }}-->
                    ...
                    <span v-show="add.inputType === 'dateTime'">
                        <DatePicker v-model="add.condition3"
                                    type="date"
                                    :disabled="add.disabled"
                                    size="mini"
                    ...
                    <template v-else-if="add.inputType === 'department'">
                        <departmentSelect :checkdDepartments.sync="add.condition3"
                            ...
                            style="display: block;width: 200px;margin-right: 5px;"></departmentSelect>
                    </template>
                </div>
    ...
    export default {
            ...
            computed_addCondition() {
                return this.addCondition.filter(item => item.condition1 && item.condition2 && ((item.condition3 && item.condition3.length) || item.condition3 instanceof Date || (typeof item.condition3 === 'number' && !isNaN(item.condition3)))).map(item => {
                    ...
                    switch (item.inputType) {
                        ...
                        //     const start = {};
                        //     start[con] = {};
                        //     start[con].$gte = this.$options.filters.timeFormat(item.condition3);
                        //     temp.$and.push(start);
                        //     const end = {};
                        //     end[con] = {};
                        //     end[con].$lte = `${this.$options.filters.timeFormat(item.condition3)} 23:59:59.999`;
                        //     temp.$and.push(end);
                        //     // temp[con][type] = new Date(item.condition3).getTime();
                        //     return temp;
                        case 'dateRange':
                ...
                data.forEach(item => {
                            ...
                            if (con1) {
                                ...
                                this.$nextTick(() => {
                                    if (this.columnType === '12') {
                                        ...
                                        this.addCondition[index].condition2 = Object.keys(valObj)[0] === '$gte' ? 'dateRange' : Object.keys(valObj)[0];
                                    } else {
                                        const valObj = Object.values(arr[0])[0] || '';
                                        this.addCondition[index].condition2 = Object.keys(valObj)[0] === '$gte' ? '$range' : Object.keys(valObj)[0];
                                    }
                                    ...
                                    this.$nextTick(() => {
                                        if (this.addCondition[index].inputType === 'numberRange') {
                                            ...
                                            if (['$range', 'dateRange'].includes(this.addCondition[index].condition2)) {
                    ...
                    if (con1) {
                        ...
                        this.$nextTick(() => {
                            ...
                            let temp_con2 = Object.keys(valObj)[0];
                            if (['countryArea', 'cpCountryArea', 'countryOfNumber'].includes(con1)) {
        ...
        watch: {
                                ...
                                if (option) {
                                    if (!this.edit_search_name) {
                                        ...
                                        if (localStorage.getItem('leadsCloud-filter-title')) {
                                            this.custom_search_name = localStorage.getItem('leadsCloud-filter-title');
                                        } else {
                ...
                handler(val) {
                    if (!val) {
                        ...
                        if (this.$refs.departmentSelect) {
                            this.$refs.departmentSelect.forEach(item => item.doClose());
                        }
            ...
            conditionList1: {
                deep: true,
                handler() {
                    // wahtsapp  会默认设置tab为自定义检索  这时候没有触发active回填表单域名所以改为监听之后回填一下
                    if (this.columnType === '12' && !this.addCondition[0].conditionList1.whatsappField) {
                        this.addCondition = [
                            {
                                condition1: '',
                                conditionList1: this.$deepClone(this.conditionList1),
