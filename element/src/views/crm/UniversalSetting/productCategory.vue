    ...
    <div>
        <div style="padding: 0 30px;">
            ...
            <div style="margin: 30px 0 12px 0;text-align: right;color: #fb8b2f;">
                <Button type="text"
                        @click="set_defaultValue"
                        style="color: #3B78DE;font-size: 12px;padding: 5px 20px 6px;">{{ $t('crm.UniSet.setDefaultValue') }}</Button>
                <Button type="text"
            ...
            <div class="table">
                <Table :data="list_options"
                    ...
                    <TableColumn prop="attrName"
                                 :label="$t('crm.UniSet.productCategory')"
                                 :show-overflow-tooltip="true">
                        <template slot-scope="scope">
                            <div class="container-productCategory-attrName">
                                {{ scope.row.attrName }}
                                <!-- 默认值标签 -->
                            </div>
                            <Tag v-if="scope.row.attrId === attrId_defaultValue"
                                 :title="$t('defaultValue')"
                                 class="tag-defaultValue">{{ $t('default') }}</Tag>
                        </template>
                    </TableColumn>
                    <!-- 操作 -->
        ...
        <Modal :visible.sync="visible"
               :title="$t('crm.UniSet.productCategory')"
               :append-to-body="true"
               width="488px"
        ...
        <!-- 设置默认值 -->
        <Modal :visible.sync="visible_defaultValue"
               :title="$t('crm.UniSet.setDefaultValue')"
               :append-to-body="true"
               v-loading="loading_defaultValue"
               width="520px">
            <Form label-position="right"
                  label-width="150px">
                <FormItem :label="$t('crm.UniSet.productCategory')">
                    <Select v-model="temp_attrId_defaultValue"
                            class="select-set-defaultValue">
                        <Option v-for="item in available_list_options"
                                :key="item.attrId"
                                :value="item.attrId"
                                :label="item.attrName"></Option>
                    </Select>
                          <!-- type="close-circled" -->
                    <Icon v-if="available_list_options.find(item => item.attrId === temp_attrId_defaultValue)"
                          :size="14"
                          @click="temp_attrId_defaultValue = null"
                          class="custom custom-shanchu1 icon-remove-defaultValue"></Icon>
                </FormItem>
            </Form>
            <div slot="footer">
                <Button @click="cancel_set_defaultValue">{{ $t('crm.UniSet.modal_cancel') }}</Button>
                <Button type="primary"
                        @click="ok_set_defaultValue">{{ $t('crm.UniSet.modal_ok') }}</Button>
            </div>
        </Modal>
    </div>
</template>
    ...
    export default {
        ...
        computed: {
            ...
            available_list_options() {
                return Array.isArray(this.list_options) ? this.list_options.filter(item => item.isAvailable === 1) : [];
            }
        },
        data() {
            return {
                ...
                /* 编辑成交数据 */
                attrId_defaultValue: null,
                temp_attrId_defaultValue: null,
                visible_defaultValue: false,
                loading_defaultValue: false
            };
        },
        methods: {
            /* 全局 */
            ...
            refreshData() {
                ...
                util.ajax({
                    ...
                    if (response.data.code === '1') {
                        // 获取默认值
                        const defaultValueOption = response.data.data.find(option => option.defaultMark === 1);
                        this.attrId_defaultValue = defaultValueOption ? defaultValueOption.attrId : null;
            ...
            /* 设置默认值 */
            set_defaultValue() {
                this.loading_defaultValue = false;
                this.visible_defaultValue = true;
            },
            ok_set_defaultValue() {
                let attrId = this.temp_attrId_defaultValue;
                let defaultMark = 1;
                const temp = this.available_list_options.find(item => item.attrId === this.temp_attrId_defaultValue);
                if (!temp) {
                    attrId = this.attrId_defaultValue;
                    defaultMark = 0;
                }
                if (attrId) {
                    this.loading_defaultValue = true;
                    util.ajax({
                        url: 'crm/attr/setDefaultMark',
                        method: 'post',
                        data: {
                            orgId: this.enterpriseId,
                            typeId: this.typeId,
                        ...
                        if (response.data.code === '1') {
                            this.visible_defaultValue = false;
                            if (defaultMark === 1) {
                                this.$Message.success(this.$t('crm.UniSet.success_set_defaultValue'));
                            } else {
                                this.$Message.success(this.$t('crm.UniSet.success_set_emptyDefaultValue'));
                            }
                            this.refreshData();
                            // 同步更改选项到线索模板和多语言
                            const temp = this.list_options.filter(item => item.isAvailable === 1).map(item => item.attrName);
                            ...
                            if (response.data.message === '停用字段不可设置为默认值') {
                                this.$Message.error(this.$t('crm.UniSet.error_setDisabledOptionDefaultValue'));
                                this.refreshData();
                            } else {
                                this.$Message.error(this.$t('crm.UniSet.error_set_defaultValue'));
                            }
                        }
                        this.loading_defaultValue = false;
                    });
                } else {
                    this.$Message.error(this.$t('crm.UniSet.success_set_emptyDefaultValue'));
                    this.visible_defaultValue = false;
                }
            },
            cancel_set_defaultValue() {
                this.visible_defaultValue = false;
                this.temp_attrId_defaultValue = this.attrId_defaultValue;
            },
            ...
            /* 操作列表项 */
            deleteAttr(attrId) {
                if (attrId === this.attrId_defaultValue) {
                    this.$Message.error(this.$t('crm.UniSet.error_deleteDefaultValue'));
                    return;
                }
            ...
            change_available_fn(id, val) {
                util.ajax({
                    ...
                    if (response.data.code === '1') {
                        ...
                        if (response.data.message === '更新失败,默认字段不支持停用！') {
                            this.$Message.error(this.$t('crm.UniSet.error_deleteDefaultValue'));
                            this.refreshData();
                        } else {
        ...
        watch: {
            ...
            attrId_defaultValue: {
                handler(val) {
                    this.temp_attrId_defaultValue = val;
                }
            }
        }
    };
...
<style lang="less">
    .tag-defaultValue {
        border-color: rgba(250,130,65,0.30);
    ...
    .container-productCategory-attrName {
