            ...
            <Poptip trigger="hover"
                ...
                <div>
                    <ul class="_font_size12">
                        <!-- 上传文件 -->
                        <li v-if="!isHighSeas && uploadFile"
                            @click="$emit('triggerUpload', item.folderId)"
                            class="piwik_companyDetail_uploadDocument">{{ $t('crm.Html.uploadDocument') }}</li>
                        <!-- 全部选中 -->
                        <li v-if="!isHighSeas"
                            @click="allChecked">{{ $t('crm.Html.selectAll') }}</li>
                        <!-- 重命名 -->
                        <li v-if="!isHighSeas"
                            @click="$emit('showAddFolder', {folderId: item.folderId, folderName: item.folderName})">{{ $t('crm.Html.rename') }}</li>
                        <!-- 删除 -->
                        <li v-if="ableDelete && !isHighSeas"
                            ...
                            <Poptip v-if="!file.uploading"
                                ...
                                <div>
                                    <ul class="_font_size12">
                                        <!-- 移动 -->
                                        <li v-if="!isHighSeas"
                                            @click="move(file)">{{ $t('crm.Html.move') }}</li>
                                        <!-- 下载 -->
                                        <li v-if="!isHighSeas"
                                            @click="download(file)"
