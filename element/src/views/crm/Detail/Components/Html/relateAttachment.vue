<template>
    <div class="communication_accessories_list">
        <RadioGroup v-model="current_compo"
                @change="wayChange">
            <RadioButton v-for="(item, index) in compo_list" :key="index" :label="$t(`crm.newDetail.module_${item}`)"></RadioButton>
        </RadioGroup>
        ...
        <div class="document_file_list"
            ...
            <p v-if="noMore && realTotal >= 100 && this.current_compo === this.$t('crm.newDetail.module_WhatsApp')"
...
<script>
    ...
    import FileList from '@/views/crm/Detail/Components/Html/fileListInquiry';
