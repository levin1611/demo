    ...
    <div>
        <Modal :title="$t('crm.Modal.title_throw')"
            ...
            <div slot="footer">
                ...
                <Button class="piwik_throw_inquiry" type="primary" @click="ok">{{ $t('crm.Modal.modal_ok') }}</Button>
            </div>
        </Modal>
    </div>
</template>
        ...
        methods: {
            ...
            getThrowReasonList() {
                ...
                util.ajax({
                    ...
                    if (response.data.code === '1') {
                        this.throwReasonList = response.data.data.filter(item => {
