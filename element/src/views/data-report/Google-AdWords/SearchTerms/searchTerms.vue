<template>
    <div class="adgroup">
<!--        <h1 class="title">{{ $t('GA.report_searchTerms') }}</h1>-->
        <div class="table_wrap">
            ...
            <div class="table">
                ...
                <Table :data="data1"
                    ...
                    <TableColumn v-for="item in table_columns"
                        ...
                        <template slot-scope="scope">
                            <template v-if="item.key === 'searchword'">
                                <a :href="`https://www.google.com.sg/search?q=${scope.row.searchword}&tbm=isch`"
                                   target="_blank"
                                   class="keywordLink">{{scope.row.searchword}}</a>
                            </template>
                ...
                <div style="margin: 10px 10px 0;overflow: hidden">
                    <div class="pagewrap">
                        <!-- <Page :page-size-opts="pagesizeopts" @on-page-size-change="pagesizechange" :total="totalpage" :current="curpage" @on-change="changePage" show-total show-sizer show-elevator placement="top"></!-->
                        <Page
        ...
        methods: {
            ...
            initdata() {
                for (let i = 0; i < this.allreportdata.length; i++) {
                    // 广告网络
                    let queryMatchTypeWithVariant;
                    switch (this.allreportdata[i].queryMatchTypeWithVariant) {
                        case 'auto':
                            queryMatchTypeWithVariant = this.$t('GA.matchTypeType.AUTO');
                            break;
                        case 'broad':
                            queryMatchTypeWithVariant = this.$t('GA.matchTypeType.BROAD');
                            break;
                        case 'exact':
                            queryMatchTypeWithVariant = this.$t('GA.matchTypeType.EXACT');
                            break;
                        case 'phrase':
                            queryMatchTypeWithVariant = this.$t('GA.matchTypeType.PHRASE');
                            break;
                        case 'exact (close variant)':
                            queryMatchTypeWithVariant = this.$t('GA.matchTypeType.NEAR_EXACT');
                            break;
                        case 'phrase (close variant)':
                            queryMatchTypeWithVariant = this.$t('GA.matchTypeType.NEAR_PHRASE');
                            break;
                        default:
                            queryMatchTypeWithVariant = this.allreportdata[i].queryMatchTypeWithVariant;
                    }

                    this.alldata1.push({
                        searchword: this.allreportdata[i].query,
            ...
            exportData() {
                ...
                const excel = this.exportExcel(this.$refs.table.$el, `${that.$t('GA.report_searchTerms')}.xlsx`);
                if (excel) {
                    console.log(excel);
                    // this.$notify.success('导出成功');
                }
                // this.$refs.table.exportCsv({
                //     filename: that.$t('GA.report_searchTerms'),
            ...
            conditiongroup(con1, con2, con3) {
                ...
                switch (con1) {
                    case this.$t('GA.searchTerms'):
                        this.keysearch('searchword', con2, con3);
                        break;
                    case this.$t('GA.matchType'):
                        this.keysearch('queryMatchTypeWithVariant', con2, con3);
                        break;
                    case this.$t('GA.cost'):
