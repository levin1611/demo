<template>
  <div class="b-r">
    <Card class="table_card _b_r" body-style="height:376px;padding: 20px 30px;">
            ...
            <!-- </template> -->
                 <template slot-scope="scope">
                    <template v-if="item.key === 'firstDepartmentName'">
                        {{ scope.row[item.key] }}
                        <span v-if="!showNextData.includes(scope.row.firstDepartmentId)" class="cursor-pointer" @click="showNext(item.key,scope.row)" >+</span>
                        <span v-if="showNextData.includes(scope.row.firstDepartmentId)" class="cursor-pointer" @click="showNext(item.key,scope.row)" >-</span>
                    </template>
    ...
    export default {
        ...
        methods: {
            ...
            showNext(column, row) {
                ...
                setTimeout(() => {
                    if (this.showNextData.length) {
                        const index = this.showNextData.findIndex(item => item === row.firstDepartmentId);
                        if (index > -1) {
                            this.showNextData.splice(index, 1);
                            setTimeout(() => {
                                this.loading = false;
                            }, 500);
                        } else {
                            this.showNextData.push(row.firstDepartmentId);
                            // this.tableColumns = this.$deepClone(this.tableColumnsAll);
                        ...
                        if (this.showNextData.length === 0) {
                            this.tableColumns = this.$deepClone(this.tableColumnsOld);
                            this.refresh_table_flag = false;
                            this.$nextTick(() => {
                                this.refresh_table_flag = true;
                                setTimeout(() => {
