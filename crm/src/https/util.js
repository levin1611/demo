...
let ajaxUrl = VUE_APP_API_URL ? VUE_APP_API_URL : 'https://qa-crm.leadscloud.com'; // 'http://43.142.47.170:9087'; // 
console.log(VUE_APP_API_URL, process.env, 'process.env环境配置');
let insDomain = 'https://instagram.leadscloud.com/'; // 七牛资源域名
...
function jump_to_login(message, before_func) {
    ...
    console.log('当前路由名:', router.currentRoute.value.name);
    const currRouteName = router.currentRoute.value.name;
    // const currRouteName = 'login';
...
const request_interceptor_success = config => {
    ...
    if (Cookies.get('accessToken')) {
        ...
        // config.headers.token = Cookies.get('accessTokenStepOne');
    }
    // 暂存当前路由名称
...
const response_interceptor_success = response => {
    ...
    if (['NEED_REDIRECT', 401, 403].includes(response.data.code)) {
        if (!['otherRouter', 'login'].includes(currRouteName)) {
            ...
            jump_to_login(t('tokenExpired'), () => {
                // 追加错误信息到 localStorage , 跳转到登录页之后打印错误信息, 并清除此 localStorage
                let quitReason = localStorage.getItem('quitUnexpectedlyReason') || '';
                quitReason += 'CODE: NEED_REDIRECT || 401 || 403;';
                localStorage.setItem('quitUnexpectedlyReason', quitReason);
            });
        }
...
const response_interceptor_error = error => {
    ...
    if (error && error.response) {
        if ([401, 403].includes(error.response.status)) {
            error.response.data.messageShow = t('tokenExpired');
            ElMessage.closeAll();
            jump_to_login(t('tokenExpired'), () => {
                // 追加错误信息到 localStorage , 跳转到登录页之后打印错误信息, 并清除此 localStorage
                // let quitReason = localStorage.getItem('quitUnexpectedlyReason') || '';
