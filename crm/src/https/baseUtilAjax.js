...
import { CODESTATE } from './codeConfig';
...
export class Request {
    ...
    async init(success = null, error200 = null, callBacks = []) {
        if (!this.methodModule[this.type]) {
            throw new Error('Allowed request methods includes [get, post, delete, put, patch]');
        }
        const response = await this.axios({ ...this.methodModule[this.type], ...this.config }).catch(err => err);
        if (typeof success === 'function' && ((response.data && response.data.code === CODESTATE.SUCCESS) || response.status === 200)) this.success(success, response.data || response, response);
        if (error200 && response.data.code !== CODESTATE.SUCCESS) this.er(error200, response.data, response);
        if (Array.isArray(callBacks) && callBacks.length) this.cbs(callBacks, response.data, response);
        // 返回正常接口后
        if ((response.data && response.data.code === CODESTATE.SUCCESS) || response.status === 200) {
