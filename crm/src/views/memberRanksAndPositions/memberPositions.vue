<template>
    <div class="position-container">
        <elInput v-model="input" :placeholder="$t('rankPosition.pleaseEnterPosition')" class="position-input" maxlength="50"/>
        <elButton style="height: 36px;" type="primary" @click="addAttrName">{{ $t('add') }}</elButton>

        <elTable
            :data="tableData"
            class="member-position-table"
        >
            <elTableColumn
...
const editAttrName = (row) => {
    if (attrNameInput.value) {
        ElMessage.error(t('rankPosition.pleaseSaveOther2'));
        return;
    }
    if (attrNameInput.value) {
        return;
...
const addAttrName = () => {
    if (!input.value) {
        ElMessage.error(t('rankPosition.positionCannotBeEmpty'));
        return;
    }
