        ...
        <main class="add_form_main">
            <div class="ruleFormBox">
                <el-form :model="ruleForm"
                    ...
                    <el-form-item :label="$t('clueConversion.conditionformName3')" prop="availObjectApiKeys" v-if="objectApiKey === 'touchPointContact'">
                        <el-checkbox-group v-model="ruleForm.availObjectApiKeys">
                            <el-checkbox :label="$t('clueConversion.conditionLabel')" value="lead" />
                            <el-checkbox :label="$t('clueConversion.conditionLabe2')" value="customer" />
                        </el-checkbox-group>
                    </el-form-item>
...
const validAvailObjectApiKeys = (rule, value, callback) => {
    if (!value.length) {
        callback(new Error(t('clueConversion.errorTip1')));
    } else {
        callback();
    }
};
...
const rules = reactive({
    ...
    availObjectApiKeys: [
        { required: true, message: t('clueConversion.errorTip1'), trigger: 'blur'},
        { validator: validAvailObjectApiKeys, trigger: 'change'}
    ],
    errorMsg: [
...
const getRuleData = () => {
    getConversionRule(props.id).then(res => {
        if (res.code == 10000) {
            ...
            ruleForm.availObjectApiKeys = ruleForm.availObjectApiKeys ? ruleForm.availObjectApiKeys : [];
...
if (props.id) {
    ...
    getKey();
    if (props.objectApiKey === 'touchPointContact') {
        ruleForm.availObjectApiKeys = ['lead', 'customer'];
    }
}
// 校验条件设置相关
...
const submitForm = async (formEl) => {
    ...
    await formEl.validate((valid) => {
        if (valid) {
            ...
            data.allSelectedFlag = data.availObjectApiKeys.length === 2 ? 1 : 0;
            data.availObjectApiKeys = data.availObjectApiKeys.length ? data.availObjectApiKeys : null;
